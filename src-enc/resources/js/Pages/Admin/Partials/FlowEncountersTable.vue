          ...
          <div class="searchToolTip">
              ...
              <div v-if="searchFilter['searchStatus']"
                   class="text-gray-600 text-sm font-semibold px-1.5 py-1 rounded bg-red-300 mb-1 w-fit pill">
                    <div class="searchTool">
                        <div><h5>Status: {{ searchFilter['searchStatus']['name'] }}</h5></div>
      ...
      >
        ...
        <template #body="slotProps">
            ...
            <div v-if="item.field==='channel_types_name'">
                <span
                      class="text-gray-600 text-xs font-semibold px-1.5 py-1 rounded mb-1 w-fit whitespace-nowrap"
                      :class="{
                            'bg-blue-300': slotProps.data[item.field] === 'Face to Face',
                            'bg-orange-300': slotProps.data[item.field] === 'Virtual',
                            'bg-green-300': slotProps.data[item.field] === 'Phone',
                         }"
                >
            ...
            <div v-if="item.field==='attendees_count'">
                <div v-for="attendees in slotProps.data[item.field]">
            ...
            <div v-if="item.field==='place_name'">{{
                ...
                <div v-if="slotProps.data['location']">({{ slotProps.data['location'] }})</div>
                </div>
...
export default defineComponent({
    ...
    methods: {
        ...
        showResult(payload) {
            this.$inertia.get(route('admin.flows.encounters', {uuid: this.flowUuid}), payload, {
                preserveState: true,
            });
        },
