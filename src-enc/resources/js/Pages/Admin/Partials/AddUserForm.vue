    ...
    <jet-dialog-modal :show="creating" modal-id="user" @close="closeModal">
      ...
      <template #content>
        <div class="grid grid-cols-1 gap-2 p-6">
            ...
            <div class="mt-4">
                <JetLabel for="roles" value="Users" mandatory="true"/>
            </div>
            <span class="p-fluid">
                <AutoComplete
                    ref="autoComplete"
                    class="" :multiple="true" v-model="form.selected_users" name="selected_users"
                ...
                />
                <JetInputError :message="form.errors.selected_users" class="mt-2"/>
            </span>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const create = () => {
            form.post(route("admin.pools.users.store", {uuid: props.pool.uuid}), {
                onSuccess: (res) => {
                    closeModal();
                },
            });
        ...
        const searchUser = (event) => {
            setTimeout(() => {
                let selectedUsers = form.selected_users ? form.selected_users : [];
                let uuids = [];
                selectedUsers.forEach((item, index) => {
                    uuids.push(item.uuid)
                })
                axios.get(route('admin.pools.users.search', {
                    uuid: props.pool.uuid,
                    search: event.query,
                    selected: uuids
                })).then(response => {
