<template>
    <span>
        <DataTable scrollHeight="80%" :value="tableData" class="mt-3" rowEditor="true" responsive="true"
                   columnResizeMode="fit" responsiveLayout="scroll" scrollDirection="horizontal" @sort="onSort($event)">
            <template #header>
            ...
            <Column v-for="item of items" :rowEditor="true" :field="item.field" :header="item.header" :key="item.field"
                    :sortable="(item.header !== 'Requestor') && (item.header !== 'Channel') && (item.header !== 'Status')"
                ...
                <template #body="slotProps">
                    <div v-if="item.field === 'uuid'">
                        ...
                        <jet-button class="ml-2 bg-blue-300" @click="copyUuid(slotProps.data[item.field])"
                    ...
                    <div v-if="item.field === 'tsm_current_state'">
                        <span v-if="slotProps.data[item.field] === 'Attention Required'"
                              class="text-gray-600 text-xs font-semibold px-1.5 py-1 rounded bg-red-300 mb-1 w-fit">
                     {{ slotProps.data[item.field] }}
                 </span>
                  <span v-if="slotProps.data[item.field] === 'Awaiting Confirmation'"
                        class="text-gray-600 text-xs font-semibold px-1.5 py-1 rounded bg-orange-300 mb-1 w-fit">
                     {{ slotProps.data[item.field] }}
                 </span>
                  <span v-if="slotProps.data[item.field] === 'Confirmed'"
                        class="text-gray-600 text-xs font-semibold px-1.5 py-1 rounded bg-green-300 mb-1 w-fit">
                     {{ slotProps.data[item.field] }}
                 </span>
                  <span v-if="slotProps.data[item.field] === 'Cancelled'"
                        class="text-gray-600 text-xs font-semibold px-1.5 py-1 rounded bg-red-600 mb-1 w-fit">
                     {{ slotProps.data[item.field] }}
                 </span>
                  <span v-if="slotProps.data[item.field] === 'Finished'"
                        class="text-gray-600 text-xs font-semibold px-1.5 py-1 rounded bg-blue-300 mb-1 w-fit">
                     {{ slotProps.data[item.field] }}
                 </span>
                    </div>
            ...
            <Column bodyStyle="text-align:right" headerClass="align-right" field="actions" header="Actions"
                    :style="{ 'max-width': 'auto', 'text-overflow': 'ellipsis', 'font-size': 'small' }">
                <template #body="slotProps">
                    <div style="display: flex;flex-direction: row;" class="justify-end">
                        <ScheduleManagementDetail :encounter="slotProps.data">
                            <jet-button-small dusk="view-button" id="view-button"
                                              class="ml-2 justify-end bg-brand-800 hover:bg-brand-900 edit-user">View
                            </jet-button-small>
...
import ScheduleManagementDetail from "./ScheduleManagementDetail.vue";
...
export default defineComponent({
    ...
    methods: {
        ...
        showResult(payload) {
            this.$inertia.get(route('agent.encounters.all'), payload, {
                preserveState: true,
            });
        },
